...
## rfid_task.c（读卡任务）
- RFID_Task：五态流转；WAIT 真实时间差超时（重发 2 次后放弃）；NONE 200ms 探测；LEDLIGHT 800ms 后 10ms 轮询；数据经 RfidLogic_Process 决策
- RFID_HandleCardData：事件执行（置 motor_trigger_flag / TTS_Send + UpdateSpeak）
- RFID_Speak / BufClear：播报与缓冲清理（上限 RFID_BLOCK_SIZE）

## motor_task.c（电机任务）
- Motor_Task：20 次采样 → CalcStopTime → 每拍 MotorLogic_Step（传入 trigger 后清零标志）→ Motor_Control；状态变化边沿输出
