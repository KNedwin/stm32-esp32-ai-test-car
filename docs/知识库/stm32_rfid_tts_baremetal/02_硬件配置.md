...
| USART2 | CN-TTS 语音模块 | 9600，8N1，printf 重定向（调试勿用） | ✅代码确认 |
| USART3 | 数据输出 + **串口配置 CLI**（2026-08 更新） | 115200，8N1；TX 轮询发送，RX 由 ParamCli_Init 置 CR1.RE 后主循环轮询收包（非中断） | ✅代码确认 |
| TIM2 | 电机 PWM（正反转经 CH1/CH2 角色交换） | 1kHz，PSC=71，ARR=999，PWM2 双通道 OCFast ENABLE | ✅代码确认 |
| ADC1 | 电位器——**已退役**：外设仍初始化并编译，Get_ADC_Value 无调用方（2026-08 更新） | IN9(PB1)，28.5 cycles，连续转换 | ✅代码确认 |
| GPIO | LED×3 | 推挽+上拉，低电平亮 | ✅代码确认 |
| 内部 Flash 末页 | 参数存储（2026-08 新增） | 0x0800FC00 起 1KB，整页擦除(FLASH_TYPEERASE_PAGES)+按字编程 | ✅代码确认 |
...
| PB1 | ADC1_IN9（电位器——硬件退役，引脚仍配为模拟输入但无采样调用方）（2026-08 更新） | 模拟输入 | ✅代码确认 |
| PB10 | USART3_TX（调试输出 + CLI 应答） | AF_PP，高速 | ✅代码确认 |
| PB11 | USART3_RX（调试口复用为串口配置 CLI 接收，ParamCli_Init 补开 RE 位）（2026-08 更新） | 输入，无上下拉 | ✅代码确认 |
