# 启动顺序与任务
## 启动顺序（main）
HAL_Init → SystemClock_Config → MX_GPIO/ADC1/TIM2/USART1/USART2/USART3 → LED_Sta(0)/PWM_Init/Motor_Control(0)/Dbg_Init → Motor_Init + RFID_Init（TTS 设置+参数）→ while(1){RFID_Process; Motor_Process}
