...
## 1. 项目概述
stm32_rfid_tts_baremetal 是"读卡语音播报程序源码 4.0 案例"的复刻工程（裸机版，芯片 STM32F103C8T6），用于新能源小车比赛场景：刷 NFC 卡（U13T 模块）→ TTS 播报卡内 GBK 中文 + 电机联动（触发词停车/定时降速/电位器停止）。
...
# 烧录
st-flash write build/stm32_rfid_tts_baremetal.bin 0x08000000
# 数据输出口监视
